    ...
    def generate_plot(self, output_file=None):
        ...
        for percentage in percentages:
            if self.selected_power_density in self.data[percentage]:
                NIR_value = self.data[percentage][self.selected_power_density]['NIR_avg_pop']
                NIR.append(NIR_value)

                blue_value = self.data[percentage][self.selected_power_density]['blue_avg_pop']
        ...
        fig.add_trace(go.Scatter(
            x=percentages, y=NIR, mode='lines+markers', name='NIR emission', line=dict(color='red'), marker=dict(size=8)
        ))
        fig.add_trace(go.Scatter(
            x=percentages, y=Blue, mode='lines+markers', name='blue emission', line=dict(color='blue'), marker=dict(size=8)
        ))
