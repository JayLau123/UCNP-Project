...
def get_constants_Er():
   ...
   return s0, beta, constant, E_phonon, threshold, n, Yb_energy, Er_energy, Yb_g, Er_g, Er_RME, Er_Omega


def get_constants_Tm():
   ...
   return s0, beta, constant, E_phonon, threshold, n, Yb_energy, Tm_energy, Yb_g, Tm_g, Tm_RME, Tm_Omega
...
class UpConversion(EnergyTransfer):
   ...
   # each element in resulting_states is a tuple, so result1[2] is the third one in the tuple: rate
   def add_state(self, ion12, ion22, rate):
   ...
   def select_path(self, r):
      ...
      # new_state is the selected index
      new_state = np.random.choice([i for i in range(len(self.resulting_states))], p=results)

      # return the selected change
      #  Note that there are many tuples stored in `resulting_states` with the form: `(ion12, ion22, rate)`, each tuple represents a choice. 
      # The method returns the selected tuple `(ion12, ion22, rate)` from `resulting_states[new_state]`, note that `new_state` is an index,`[0:2]` means the first two elements, which also return a tuple: `(ion12, ion22)`, 
      # that means the final state of donor and acceptor. `(ion12, ion22)` would be used to update the ion's states.

      return self.resulting_states[new_state][0:2]
   


def up_conversion_Er():
   
   s0, beta, constant, E_phonon, threshold, n, Yb_energy,  E_levels, Yb_g, g_value, RME_value, Omega_value = get_constants_Er()
...
def up_conversion_Tm():
   
   s0, beta, constant, E_phonon, threshold, n, Yb_energy,  E_levels, Yb_g, g_value, RME_value, Omega_value = get_constants_Tm()
...
def cross_relaxation_Er():
   ...
   for ion1_energy in E_levels:
      ...
      for ion2_energy in E_levels:
         ...
         all_transitions = {}
         delta_E_ion1 = {f'{ion1_energy}{level}':  E_levels[ion1_energy] -  E_levels[level] for level in E_levels if level != ion1_energy}
         delta_E_ion2 = {f'{ion2_energy}{level}': E_levels[ion2_energy] - E_levels[level] for level in E_levels if level != ion2_energy}
