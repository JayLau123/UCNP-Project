...
def get_constants():
   ...
   Impurity_RME={'e1e2':[1e-18], 'e1e3':[1e-18], 'e1e4':[1e-18], 'e1e5':[1e-18], 'e1e6':[1e-19], 'e1e7':[1e-19]}
   ...
   Ligand_energy = {'e1':0, 'e2':1700, 'e3':3000, 'e4':4300, 'e5':6000, 'e6':7500}
   ...
   return s0, beta, constant, E_phonon, threshold, Er_RME, Er_energy, Yb_energy, Er_g, Yb_g, Er_Omega, n, Ligand_energy, Impurity_RME
...
def up_conversion():
   ...
   s0, beta, constant, E_phonon, threshold, Er_RME, Er_energy, Yb_energy, Er_g, Yb_g, Er_Omega, n, Ligand_energy, Impurity_RME = get_constants()
   ...
   for ion2_energy in Er_energy:
      ...
      for transition2, energy_diff2 in delta_E_ion2.items():
         ...
         if (energy_diff2 < 0 and abs(10246 + energy_diff2)< n*E_phonon):
            ...
            ET_key = f'S1S0-{transition2}'
            all_transitions[ET_key] = Delta_E
         
      for ET_key in all_transitions:
            first_part, second_part = ET_key.split('-')
            second_values = RME_value[second_part]
            all_transitions[ET_key] = [all_transitions[ET_key], 1.5*10**-20, second_values]
...
class BackTransfer(EnergyTransfer):
   def __init__(self, ion2):
      self.ion2 = ion2
      self.resulting_states = []

   def total_probability(self, r):
      return sum([result[2]/(r/10**7)**6 for result in self.resulting_states])

   def select_path(self, r):
      if len(self.resulting_states) == 0:
         return None
      
      results = [result[2]/(r/10**7)**6 for result in self.resulting_states]
      results = [prob / sum(results) for prob in results]
      new_state = np.random.choice([i for i in range(len(self.resulting_states))], p=results)
      return self.resulting_states[new_state][0:2]
...
def back_transfer():
   ...
   for ion2_energy in list(Er_energy.keys())[2:]:  # Start from Er's 2nd excited state onwards
      ion2_parts = ion2_energy.split('E')
      ion2_initial_state = int(ion2_parts[1])
      ion2_et = BackTransfer(ion2_initial_state)

      all_transitions = {}
      delta_E_ion1 = {'S0S1': 10246}  # Yb energy difference
      delta_E_ion2 = {f'{ion2_energy}E{level}': E_level[ion2_energy] - E_level[f'E{level}'] for level in range(ion2_initial_state)}

      for transition2, energy_diff2 in delta_E_ion2.items():
         if abs(energy_diff2 - delta_E_ion1['S0S1']) < n * E_phonon:
            Delta_E = abs(energy_diff2 - delta_E_ion1['S0S1'])
            ET_key = f'{transition2}-S0S1'
            all_transitions[ET_key] = Delta_E

      for ET_key in all_transitions:
         first_part, second_part = ET_key.split('-')
         first_values = RME_value[first_part]
         all_transitions[ET_key] = [all_transitions[ET_key], 1.5*10**-20, first_values]
      ...
      for key, value in all_transitions.items():
         ...
         if my_value > threshold:
            ...
            donor_final_state = int(donor_parts[2])
            acceptor_parts = acceptor_transition.split('S')
            acceptor_final_state = int(acceptor_parts[2])
...
class ET_Impurity(EnergyTransfer):
   def __init__ (self, ion1, ion2):
      self.ion1 = ion1
      self.ion2 = ion2
      self.resulting_states = []
...
def et_impurity():
   ...
   s0, beta, constant, E_phonon, threshold, Er_RME, Er_energy, Yb_energy, Er_g, Yb_g,  Er_Omega, n, impurity_energy, Impurity_RME = get_constants()

   energy_levels = Er_energy
   impurity_levels = impurity_energy
   
   RME_value_1 = Er_RME
   RME_value_2 = Impurity_RME
   ...
   for ion1_energy in Er_energy:
      ...
      for ion2_energy in impurity_energy:

         acceptor_parts = ion2_energy.split('e')
         acceptor_initial_state=int(acceptor_parts[1])

         ion1_ion2_et = ET_Impurity(donor_initial_state, acceptor_initial_state)
         ...
         for transition1, energy_diff1 in delta_E_ion1.items():
            for transition2, energy_diff2 in delta_E_ion2.items():
               ...
               ################################################################# important: ET matching condition
               if (energy_diff1 > 0 and energy_diff2 < 0 and abs(energy_diff1+energy_diff2)< n*E_phonon):
         ...
         for ET_key in all_transitions:
            ...
            # Oscillator strength: S (Emory)
            first_values = RME_value_1.get(first_part, [])
            second_values = RME_value_2.get(second_part, [])
            ...
            # append degeneracy of initial level of donor and acceptor
            all_transitions[ET_key].append(g_value[new_key1])
         ...
         for key, value in all_transitions.items():
            ...
            my_value = constant*s0*(S1*S2)*np.exp(-beta*value[0])/(value[3])
